...
아래처럼 이미지를 배경으로 넣으면 `a.png` 아래에 `b.png`가 깔리고, 맨 밑에는 `c.png`가 깔립니다.
...
기본값은 `left top`(왼쪽 위)이고, 가운데 정렬을 하려면 아래처럼 `center`를 쓰면 됩니다.
...
기본값은 `repeat`(반복)이고, `no-repeat`으로 하면 반복되지 않게 할 수 있습니다.
...
이 각도를 바꾸고 싶다면 맨 앞에다가 각도를 써 주면 됩니다. 각도의 단위는 `deg`입니다.
...
`opacity`는 요소 전체를 반투명하게 만들기 때문에, 요소의 배경만 반투명하게 만들고 싶다면 배경색을 반투명하게 만들어야 합니다.

예를 들어서 검정색 배경을 반투명하게 만들고 싶어서 아래처럼 사용한다면 해당 요소 전체가 반투명해지는데요.

```
background-color: #000000;
opacity: 0.6; /* 요소 전체가 반투명해짐 */

```

이럴 때는 아래처럼 `rgba()`를 활용해서 반투명한 배경색을 사용해 보세요.
